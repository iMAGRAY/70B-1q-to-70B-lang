...
def _default_url(url: str | None) -> str:
    """Return the provided URL or fall back to $SIGLA_SERVER."""
    if url:
        return url
    env = os.environ.get("SIGLA_SERVER")
    if env:
        return env
    raise SystemExit("error: server URL required or set SIGLA_SERVER")
...
def answer_query(
    ...
    """Retrieve capsules and generate an answer with a local LLM."""
    index_path = _default_index(index_path)
    try:
        store = CapsuleStore()
        store.load(index_path)
...
def ping_server(url: str | None) -> None:
    """Ping the SIGLA server and print the response."""
    import requests

    url = _default_url(url)

    try:
        resp = requests.get(url.rstrip("/") + "/ping", timeout=5)
...
def run_server(index_path: str, host: str, port: int, log_file: str | None) -> None:
    """Run the SIGLA FastAPI server."""
    index_path = _default_index(index_path)
    if sigserver.FastAPI is None:
    ...
    try:
        ...
        return
    sigserver.index_path = index_path
    if log_file:
        siglog.start(log_file)
    uvicorn.run(sigserver.app, host=host, port=port)
...
def main():
    ...
    answer_p = subparsers.add_parser("answer", help="retrieve context and generate an answer")
    answer_p.add_argument("index_path")
    answer_p.add_argument("query")
    answer_p.add_argument("--model-path", required=True, help="path to local LLM")
    answer_p.add_argument("--top_k", type=int, default=5)
    answer_p.add_argument("--tags")
    answer_p.add_argument("--sources")
    answer_p.add_argument("--temperature", type=float, default=0.7)
    answer_p.add_argument("--min-rating", type=float, default=0.0)
    answer_p.add_argument("--max-tokens", type=int, default=256)
    ...
    server_p = subparsers.add_parser("server", help="run API server")
    server_p.add_argument("index_path")
    server_p.add_argument("--host", default="127.0.0.1")
    server_p.add_argument("--port", type=int, default=8000)
    server_p.add_argument("--log-file")

    ping_p = subparsers.add_parser("ping", help="check server liveness")
    ping_p.add_argument("url", nargs="?", help="base server URL or $SIGLA_SERVER")
    ...
    version_p = subparsers.add_parser("version", help="show package version")
    ...
    if args.cmd == "ingest":
        ...
        run_server(args.index_path, args.host, args.port, args.log_file)
    elif args.cmd == "ping":
