...
5. Search for relevant capsules (you can filter by tags and rating):

```bash
python -m sigla search myindex "философия и счастье" --tags философия --min-rating 0.8
```
...
python -m sigla inject myindex "философия и счастье" --top_k 3 --tags философия --temperature 0.7 --min-rating 0.5
```
...
python -m sigla compress myindex "философия и счастье" --top_k 3 --tags философия --min-rating 0.5
```
...
curl "http://localhost:8000/search?query=философия&tags=философия&min_rating=0.8"
```
...
curl "http://localhost:8000/ask?query=философия&temperature=0.7&min_rating=0.8"
```
...
curl "http://localhost:8000/compress?query=философия&top_k=3&min_rating=0.8"
```
