...
```bash
python -m sigla compress myindex "философия и счастье" --top_k 3 --tags философия --sources Claude
python -m sigla compress myindex "философия и счастье" --top_k 3 --tags философия --sources Claude --min-rating 0.5
```
...
```bash
curl "http://localhost:8000/compress?query=философия&top_k=3&sources=Claude"
curl "http://localhost:8000/compress?query=философия&top_k=3&sources=Claude&min_rating=0.8"
```
