...
Run `pip install faiss-cpu sentence-transformers fastapi uvicorn transformers llama-cpp-python requests` to install optional dependencies.
...
python -m sigla server myindex  # тот же сервер через общий CLI
```
...
curl http://localhost:8000/version  # версия сервера
python -m sigla ping http://localhost:8000  # то же через CLI
export SIGLA_SERVER=http://localhost:8000  # можно не указывать URL каждый раз
python -m sigla ping  # использует $SIGLA_SERVER
```
