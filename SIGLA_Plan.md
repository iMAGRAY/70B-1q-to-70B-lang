...
- Сервер предоставляет эндпоинт `/version` для проверки версии.
- Сервер также имеет `/ping` для проверки работоспособности.
- CLI команда `ping` позволяет быстро проверить работу сервера.
- По умолчанию `sigla ping` использует адрес из переменной окружения `$SIGLA_SERVER`.
- Запустить API можно через `sigla server`, что упрощает локальный запуск.
